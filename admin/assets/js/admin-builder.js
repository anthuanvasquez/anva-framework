...
jQuery( document ).ready( function( $ ) {
    ...
    var AnvaBuilder = {
        ...
        active: function() {
            ...
            AnvaBuilder.checkTemplate( $template.val() );
            $template.on( 'change', function() {
                AnvaBuilder.checkTemplate( $( this ).val() );
            });
        },
        ...
        buildItem: function() {
            ...
            if ( $shortcode !== '' ) {
                ...
                // Get ajax URL
                var ajaxEditURL = AnvaBuilderLocal.ajaxurl + '?action=anva_builder_get_fields&shortcode=' + $shortcode + '&rel=' + $randomId;
        ...
        addItem: function() {
            $( '#add-builder-item' ).on( 'click', function(e) {
                ...
                if ( '' === $( '#anva_shortcode' ).val() ) {
                    swal({
                        title: 'Builder',
                        text: AnvaBuilderLocal.builder_empty,
                        type: "info",
        ...
        removeItem: function() {
            $(document).on( 'click', '#builder-sortable-items a.button-remove', function(e) {
                e.preventDefault();
                var $parentEle = $(this).parent( '.actions' ).parent( 'li' );
                if ( confirm( AnvaBuilderLocal.builder_remove ) ) {
                    $parentEle.fadeOut();
                    setTimeout( function() {
                        $parentEle.remove();
        ...
        removeAllItems: function() {
            $(document).on( 'click', '#remove-all-items', function(e) {
                ...
                if ( $( '#builder-sortable-items li' ).length === 0 ) {
                    swal({
                        title: 'Builder',
                        text: AnvaBuilderLocal.builder_remove_empty,
                        type: "info",
                        showConfirmButton: true,
                        confirmButtonColor: "#0085ba",
                    });
                ...
                swal({
                    title: 'Builder',
                    text: AnvaBuilderLocal.builder_remove_all,
                    type: "warning",
                    showCancelButton: true,
                    confirmButtonColor: "#0085ba",
                    confirmButtonText: AnvaBuilderLocal.confirm,
                    cancelButtonText: AnvaBuilderLocal.cancel,
        ...
        importExport: function() {
            ...
            $(document).on( 'click', '.button-export', function(e) {
                $( '#builder-sortable-items li' ).each( function() {
                    if ( $(this).hasClass( 'item-unsaved' ) ) {
                        alert( AnvaBuilderLocal.builder_unsave );
                        e.preventDefault();
                    }
                });
                $( '#anva-export' ).val(1);
            ...
            $(document).on( 'click', '.button-import', function(e) {
                if ( $( '#anva-import-file' ).val() === '' ) {
                    alert( AnvaBuilderLocal.builder_import );
                    e.preventDefault();
                }
        ...
        extras: function() {
            ...
            $(document).on( 'click', '.anva-upload-button', function(e) {
                ...
                if ( $remove === $button.text() ) {
                    ...
                    frame = wp.media({
                        title: AnvaBuilderLocal.builder_upload,
                        button: {
                            text: AnvaBuilderLocal.builder_select_image,
                            close: false
                        },
                        multiple: false
                    });
