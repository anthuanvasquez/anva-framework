jQuery(document).ready(function($) {
		...
		if ( optionsframework_upload ) {
			...
			optionsframework_upload.on( 'select', function() {
				...
				if ( attachment.attributes.type == 'image' ) {
					optionsframework_selector.find('.screenshot').empty().hide().append( '<img src="' + attachment.attributes.url + '"><a class="remove-image">X</a>' ).slideDown('fast');
				}
	...
	function optionsframework_remove_file( selector ) {
		...
		selector.find('.upload-button').on( 'click', function( event ) {
			optionsframework_add_file( event, $(this).closest('.section') );
		});
	}

	$('.remove-image, .remove-file').on( 'click', function() {
		optionsframework_remove_file( $(this).closest('.section') );
	});
